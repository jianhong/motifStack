...
Users can select different fonts and colors to draw the sequence logo (Figure \ref{fig:font}).
\begin{scriptsize}
...
it can also be used to draw amino acid sequence logos (Figure \ref{fig:protein}).
\begin{scriptsize}
...
function to draw sequence logos stack (Figure \ref{fig:logostack}) or use plotMotifLogoStackWithTree function 
to show the distance tree with the sequence logos stack (Figure \ref{fig:treestack}) or 
use plotMotifStackWithRadialPhylog function to plot sequence logo stack in radial style 
(Figure \ref{fig:radialstack}) 
...
\begin{Schunk}
\begin{Sinput}
>motifStack(motifs2, layout="radialPhylog", circle=0.3, cleaves = 0.2, clabel.leaves = 0.5, 
+                    col.bg=rep(color, each=5), col.bg.alpha=0.3, 
+                    col.leaves=rep(color, each=5),
+                    col.inner.label.circle=rep(color, each=5), inner.label.circle.width=0.05,
+                    col.outer.label.circle=rep(color, each=5), outer.label.circle.width=0.1, 
+                    circle.motif=1.2, angle=350)
\end{Sinput}
\end{Schunk}
\end{scriptsize}
\begin{figure}[htb]
...
motifStack(motifs2, layout="radialPhylog", circle=0.3, cleaves = 0.2, clabel.leaves = 0.5, 
                                 ...
                                 col.leaves=rep(color, each=5),
                                 col.inner.label.circle=rep(color, each=5), inner.label.circle.width=0.05,
                                 col.outer.label.circle=rep(color, each=5), outer.label.circle.width=0.02, 
                                 circle.motif=1.2,
                                 angle=350)
@
...
We can also plot a sequence logo cloud for DNA sequence logo (Figure \ref{fig:motifCloud}).
\begin{scriptsize}
...
To plot grouped sequence logo, except do motifCloud, we can also plot it with radialPhylog style (Figure \ref{fig:motifRadialPhylog}).
\begin{scriptsize}
<<groupedMotif,include=FALSE>>=
...
\begin{Schunk}
\begin{Sinput}
> plotMotifStackWithRadialPhylog(phylog=phylog, pfms=sig, circle=0.4, cleaves = 0.3, 
+                               clabel.leaves = 0.5, 
+                               col.bg=rep(color, each=5), col.bg.alpha=0.3, 
+                               col.leaves=rep(color, each=5),
...
plotMotifStackWithRadialPhylog(phylog=phylog, pfms=sig, circle=0.4, cleaves = 0.3, 
                              ...
                              col.inner.label.circle=pfmNames[,3], 
                              inner.label.circle.width=0.03,
                              angle=350, circle.motif=1.2, motifScale="logarithmic")
@
\caption{grouped sequence logo with radialPhylog style layout}
\label{fig:motifRadialPhylog}
\end{figure}
