...
\begin{Schunk}
\begin{Sinput}
> plotMotifStackWithRadialPhylog(phylog, pfms, circle=0.4, cleaves = 0.5, clabel.leaves = 0.5,  
...
plotMotifStackWithRadialPhylog(phylog, pfms, circle=0.4, cleaves = 0.2, clabel.leaves = 0.5, 
                                 col.bg=rep(color, each=5), col.bg.alpha=0.3, 
                                 col.leaves=rep(color, each=5),
                                 col.inner.label.circle=rep(color,5),
                                 col.outer.label.circle=rep(color,5), outer.label.circle.width=0.1, 
...
pfmNames <- gsub("(RC)","",pfmNames, fixed=T)
groups <- rep(paste("group",1:5,sep=""), each=10)
names(groups) <- pfmNames
group.col <- brewer.pal(5, "Set3")
names(group.col)<-paste("group",1:5,sep="")
@
\begin{Schunk}
\begin{Sinput}
> motifCloud(pfms, phylog, groupDistance=0.1, scale=c(6, .5), 
...
motifCloud(pfms, phylog, groupDistance=0.1, scale=c(6, .5), layout="rectangles", group.col=group.col, groups=groups, draw.legend=T)
@
