...
<<DNAseqLogo,fig=TRUE,include=FALSE,width=8,height=6>>=
library(motifStack)
...
\subsection{motifCircos}
We can also plot it with circos style (Figure \ref{motifStack-motifCircos}). In circos style,
we can plot two group of motifs and with multiple color rings.

<<motifCircos,fig=TRUE,include=FALSE,width=6,height=6>>=
## plot the logo stack with radial style.
...
@
\incfig{motifStack-motifCircos}{\textwidth}
{Grouped sequence logo with circos style layout.}
{more color sets with more motifs.}

\subsection{motifPiles}
We can also plot it with pile style (Figure \ref{motifStack-motifPiles}). In pile style,
we can plot two group of motifs and with multiple color annoations.

<<motifPiles,fig=TRUE,include=FALSE,width=6,height=6>>=
## plot the logo stack with radial style.
motifPiles(phylog=phylog, pfms=pfms, pfms2=sig, 
            ...
            groupDistance=0.01)
@
\incfig{motifStack-motifPiles}{\textwidth}
